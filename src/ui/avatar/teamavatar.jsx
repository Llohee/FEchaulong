...
const avatarStyles = cva(
  "flex justify-center items-center rounded-lg select-none",
  {
    variants: {
      size: {
        xs: "size-6 aspect-square text-[11px]",
        small: "size-9",
        medium: "size-16 text-heading-5",
        xl: "size-24 text-[50px] font-bold",
      },
      noName: {
...
const TeamAvatar = (props) => {
  ...
  const basicColors = ["#4D7AE5", "#FD7A1C", "#40BF79", "#B87DE8", "#E4584E"];
  const randomColorIndex = hashed % basicColors.length;
  const c = color(basicColors[randomColorIndex]);
  ...
  return (
    <div
      role="img"
      aria-label={props.name || "undefined teams avatar"}
      style={{
