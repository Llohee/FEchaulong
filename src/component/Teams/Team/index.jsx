...
const Team = () => {
  ...
  return (
    <>
      <div className="sub_container overflow-hidden shadow-r shadow-2xl fixed w-96 z-50 bg-slate-600 min-h-screen">
        <div className="py-4 px-2 col-span-1 flex flex-col gap-4">
          <div className="flex flex-col gap-8">
            <div className="px-4 py-2">
              <Link to={`/teams/`}>
                <button className="flex gap-1 items-center bg-slate-500 duration-100 px-2 py-2 rounded-md hover:scale-x-110 !text-white">
                  <svg
                    width="20px"
                    height="20px"
                    viewBox="-0.5 0 25 25"
            ...
            <div className="">
              ...
              <div className="h-[3px] bg-slate-300 rounded-3xl mx-4 mt-6"></div>
            </div>
          </div>
          <div classname="flex flex-col gap-2">
            <div className="flex justify-between items-center pr-4">
              <div className="text-2xl text-white">{teamById.name}</div>
              <Menu
                as="div"
              ...
              >
                ...
                >
                  <Menu.Items className="bg-slate-500 rounded-md absolute flex flex-col gap-0 right-0 origin-top-right shadow-2xl focus:outline-none z-[8000]">
                    {listMenuItems.map((val, index) => (
                      <Menu.Item key={index}>
                        <button
                          onClick={() => val.onClick?.()}
                          target="_parent"
                          className="m-1 flex items-center gap-1 hover:rounded-md px-3 py-2 hover:bg-slate-300/50 active:bg-grey-4 focus:bg-grey-4"
                        >
                          <div className="my-auto text-white">
                            {val.icon}
                          </div>
                          <div className="flex flex-col text-sm whitespace-nowrap text-white">
                            {val.title}
                          </div>
                        </button>
                      </Menu.Item>
                  ...
                  <Tab
                    ...
                    className={({ selected }) =>
                      `menu-item text-black text-md ${
                        selected
                          ? "bg-slate-300/75 rounded-md text-left px-4 py-2"
                          : "text-left px-4 py-2 hover:bg-slate-300/50 rounded-md text-white hover:text-black"
                      }`
                    }
                  >
                    {category.title}
